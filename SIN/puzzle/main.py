...
def getEuclideanDistance(state):
    ...
    return tot
# ==================================================================ini practica=================================================================================
# heuristic function using 'peces descolocades'
# calcula el numero de peces descolocades
def getDescDistance(state):
    tot = 0 
    for i in range(1,9):
        if state.index(str(i)) != end_state.index(str(i)): tot += 1
    return tot

# heuristic function using 'sequencies'
# utilitzar array per a trobar seg poscio a la que mirar
def getSecDist(state):
    tot = 0;
    nextPos = [1,2,5,0,-1,8,3,6,7]
    for i in range(0,9):
        # calcular quina deuria ser la següent fitxa
        fseg = (int(state[i])%8)+1
        # comprovar si la fintxa central es 0
        if (nextPos[i] < 0 and state[i] != 0): tot = tot + 1
        elif (state[nextPos[i]] != str(fseg)): tot = tot + 2 

    return 3*tot


# heurist function using 'Files_Columnes'

def getFilColDist(state):
    tot = 0 
    for i in range(1,9):
        goal = end_state.index(str(i))
        goalX = int(goal/ 3)
        ...
        itemX = int(idx / 3)
        itemY = idx % 3
        if itemX != goalX: tot += 1
        if itemY != goalY: tot += 1
    return tot

# heristica examen
# si la segueix la seua successora mirar si està en la posició correcta

def getSeqDesc(state):
    tot = 0;
    nextPos = [1,2,5,0,-1,8,3,6,7]
    for i in range(0,9):
        # calcular quina deuria ser la següent fitxa
        fseg = (int(state[i])%8)+1
        # comprovar si la fintxa central es 0
        if (nextPos[i] < 0 and state[i] != 0): tot = tot + 1 # si la fitxa central no es 0
        elif (state[nextPos[i]] != str(fseg)): tot = tot + 2 # si la fitxa que la segueix no es la correcta 
        elif (state.index(str(i)) != end_state.index(str(i))): tot = tot + 1
